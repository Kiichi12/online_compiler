...
def submit_code(submission: CodeSubmission):
    ...
    try:
        ...
        if(language == "python"):
            ...
            binary_file = f"{container_id}.out"
            file_path = os.path.join(tmp2_dir, code_file)
            input_path = os.path.join(tmp2_dir, input_file)
            with open(file_path, "w") as f:
                f.write(code)
            ...
            container = client.containers.run(
                image=config["image"],
                command=f"/bin/sh -c 'g++ /tmp/{code_file} -o /tmp/{binary_file} && /tmp/{binary_file} < /tmp/{input_file}'"
                if stdin_input else f"/bin/sh -c 'g++ /tmp/{code_file} -o /tmp/{binary_file} && /tmp/{binary_file}'",
                volumes={tmp2_dir: {'bind': '/tmp', 'mode': 'rw'}},
                working_dir="/tmp",
                mem_limit="128m",
                cpu_period=100000,
            ...
            container = client.containers.run(
                image=config["image"],
                command=f"/bin/sh -c 'gcc /tmp/{code_file} -o /tmp/{binary_file} && /tmp/{binary_file} < /tmp/{input_file}'"
                if stdin_input else f"/bin/sh -c 'gcc/tmp/{code_file} -o /tmp/{binary_file} && /tmp/{binary_file}'",
                volumes={tmp2_dir: {'bind': '/tmp', 'mode': 'rw'}},
                working_dir="/tmp",
                mem_limit="128m",
                cpu_period=100000,
